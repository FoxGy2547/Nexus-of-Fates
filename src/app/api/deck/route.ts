...
export async function POST(req: Request) {
  ...
  try {
    ...
    const params: (number)[] = [body.userId, ...allIds];
    const placeholders = allIds.map((_, i) => `$${i + 2}`).join(",") || "NULL";

    type StockRow = { card_id: number | string; qty: number | string };
    ...
    stockRes.rows.forEach((r) =>
      stock.set(Number(r.card_id), Number(r.qty))
    );
    ...
    for (let i = 0; i < body.characters.length; i++) {
      await client.query(
        `INSERT INTO deck_chars(deck_id, slot, card_id) VALUES($1,$2,$3)`,
        [deckId, i + 1, body.characters[i]]
      );
    }
