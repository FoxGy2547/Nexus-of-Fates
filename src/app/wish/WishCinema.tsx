...
function highestRarity(items: WishItem[]): 3 | 4 | 5 {
  ...
  for (const it of items) {
    if (it.rarity === 5) return 5;
    if (it.rarity === 4) max = 4;
  }
  return max;
}
...
export default function WishCinema({ open, results, onDone }: Props) {
  ...
  // ล็อก scroll เมื่อ overlay เปิด
  useEffect(() => {
    if (open) {
  ...
  const ribbonClass = useMemo(() => {
    const r = highestRarity(results);
    return r === 5 ? "meteor--gold" : r === 4 ? "meteor--purple" : "meteor--blue";
  }, [results]);
  ...
  useEffect(() => {
    ...
    setRevealIndex(0);
    setSkipped(false);
    const t = window.setTimeout(() => setPhase("flip"), INTRO_MS);
  ...
  return (
    <div className="wish-overlay">
      ...
      {(phase === "flip" || phase === "summary") && (
        <div className={`flip-grid columns-${results.length === 10 ? "ten" : "one"}`}>
          {results.map((it, idx) => {
            const revealed = idx < revealIndex || phase === "summary";
            const is5 = it.rarity === 5;
            const is4 = it.rarity === 4;
            return (
              <button
                key={`${it.kind}-${it.id}-${idx}`}
                className={`flip-card ${revealed ? "is-revealed" : ""} ${is5 ? "is-five" : is4 ? "is-four" : "is-three"}`}
              ...
              >
                ...
                <div className="flip-face flip-front">
                  ...
                  <div className="front-img">
                    <Image src={artPath(it)} alt={it.name || it.code} fill className="object-contain" unoptimized />
                  </div>
      ...
      <style jsx>{`
        .wish-overlay {
          ...
          inset: 0;
          z-index: 60;
          /* พื้นหลังทึบจริง + เกรเดียนต์ด้านบน */
          background:
            radial-gradient(1200px 600px at 50% 30%, rgba(35, 52, 90, 0.85), rgba(6, 10, 18, 0.98) 55%),
            #060a12;
          display: flex;
          align-items: center;
        ...
        .meteor__tail--1 { transform: rotate(22deg); }
        .meteor__tail--2 { top:-23%; height:2px; opacity:.6; transform: rotate(21deg); }
        .meteor__tail--3 { top:-27%; height:6px; opacity:.3; filter:blur(2px); transform: rotate(23deg); }
