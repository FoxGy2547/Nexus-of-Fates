...
/** กล่องภาพแนวตั้ง 2:3 (1024x1536) ย่อ/ขยายแบบไม่บิด ไม่ครอป */
function PortraitCardImage({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="relative w-full aspect-[2/3] rounded-lg overflow-hidden bg-neutral-900/60">
    ...
    <main className="min-h-screen p-6 flex flex-col gap-6">
      <header className="flex items-center gap-3">
        ...
        <div className="text-sm opacity-70">
          Others {Object.values(selOthers).reduce((a, b) => a + b, 0)}/20
        </div>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
          {CHAR_CARDS.map((c) => {
            ...
            const src = cardImg(c.code, c.art, "character");
            return (
              <button
                key={c.id}
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
          {OTHER_CARDS.map((o) => {
            ...
            const src = cardImg(o.code, o.art, o.kind);
            return (
              <div
                key={`${o.kind}-${o.id}`}
              ...
              >
                ...
                {/* ปุ่มลบบนมุมขวาบน จะโชว์เมื่อเลือกไว้แล้ว */}
                {picked > 0 && (
                  <button
                ...
                {/* กดที่การ์ด = เพิ่ม 1 ใบ */}
                <button className="block w-full text-left" onClick={() => addOther(o.id)} title="กดการ์ดเพื่อเพิ่ม 1 ใบ">
